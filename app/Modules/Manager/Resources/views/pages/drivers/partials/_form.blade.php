            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                ...
                @if(!isset($driver))
                    <div class="col-span-full">
                        <h5 class="mb-3 text-base font-semibold text-gray-700 dark:text-gray-300">Login Credentials</h5>
                    </div>
                        ...
                        <div class="relative flex gap-2">
                            <div class="relative flex-1">
                                <input
                                    ...
                                    class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 pr-10 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                                />
                                <button type="button" x-on:click="showPassword = !showPassword" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
        ...
        function driverFormComponent(initialData) {
            return {
                ...
                checkStrength() {
                    ...
                    this.passwordStrength = score <= 2 ? 'fair' : score === 3 ? 'good' : 'strong';
                },
                ...
                async submitDriverForm(url, method) {
                    ...
                    if (response.status === 201 && method === 'POST' && data.credentials) {
                        sessionStorage.setItem('toast_success', data.message ?? 'Driver created successfully.');

                        await Swal.fire({
                            title: 'Driver Created Successfully',
                            icon: 'success',
                            html: `
                                <p class="text-sm text-gray-500 mb-4">Share these login credentials with the driver. The password will not be shown again.</p>
                                <div class="text-left bg-gray-50 rounded-lg p-4 space-y-3">
                                    <div>
                                        ...
                                        <p class="text-sm font-medium text-gray-800" id="swal-email">${data.credentials.email}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs text-gray-400 uppercase tracking-wide mb-1">Password</p>
                                        <p class="text-sm font-medium text-gray-800 font-mono" id="swal-password">${data.credentials.password}</p>
                                    </div>
                                </div>
                            `,
                            ...
                            if (result.isConfirmed) {
                                const text = `Email: ${data.credentials.email}\nPassword: ${data.credentials.password}`;
